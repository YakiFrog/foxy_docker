...
do
    for i in {1..4}
    do
        echo "[Loop $j, Step $i/4] Step 1: Moving Forward (Open Loop)"
        ros2 action send_goal /open_loop_drive bt_msgs/action/Drive "{type: 'move', target_value: 2.0, speed: 1.0}"
        
        echo "[Loop $j, Step $i/4] Step 2: Rotating 90 Degrees (Open Loop)"
        ros2 action send_goal /open_loop_drive bt_msgs/action/Drive "{type: 'rotate', target_value: -90.0, speed: 1.0}"
    done
done
