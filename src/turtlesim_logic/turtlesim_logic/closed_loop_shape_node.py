...
class ClosedLoopShapeNode(Node):
    def __init__(self):
        super().__init__('closed_loop_shape_node')
        ...
        self._square_action_server = ActionServer(
            self, Square, 'closed_loop_square',
            execute_callback=self.execute_square_callback,
            callback_group=ReentrantCallbackGroup()
        )
        self._figure_eight_action_server = ActionServer(
            self, FigureEight, 'closed_loop_figure_eight',
            execute_callback=self.execute_figure_eight_callback,
            callback_group=ReentrantCallbackGroup()
        )
        self.get_logger().info('High-Precision ClosedLoopShape Node Started (Hosting Square & FigureEight Actions)')
    ...
    async def run_motion(self, drive_type, target_value, speed, radius, target_x, target_y, p_control_mode, goal_handle):
        ...
        # --- P減速を使うかどうかの決定ロジック ---
        use_p = False
        if p_control_mode == 1: # 強制 ON
            use_p = True
        elif p_control_mode == 2: # 強制 OFF
            use_p = False
        ...
        try:
            ...
            # --- 2. 相対移動 (rotate / move / arc) ---
                ...
                if drive_type == "rotate":
                    speed_base = speed if speed > 0.0 else abs(self.get_parameter('angular_speed').value)
                    limit = abs(math.radians(target_value))
                elif drive_type == "move":
                    speed_base = speed if speed > 0.0 else abs(self.get_parameter('linear_speed').value)
            ...
            self.get_logger().error(f"Error in motion: {e}")
            self.stop_turtle()
            return False
    ...
    async def execute_square_callback(self, goal_handle):
        ...
        p_control_mode = goal_handle.request.p_control_mode

        self.get_logger().info(f'Starting Closed Loop Square: side={side}, radius={radius}')

        for i in range(4):
            success = await self.run_motion("move", side, speed, 0.0, 0.0, 0.0, p_control_mode, goal_handle)
            if not success:
                if goal_handle.is_cancel_requested:
            ...
            success = await self.run_motion("arc", -90.0, speed, radius, 0.0, 0.0, p_control_mode, goal_handle)
            if not success:
                if goal_handle.is_cancel_requested:
                    goal_handle.canceled()
                else:
    ...
    async def execute_figure_eight_callback(self, goal_handle):
        ...
        self.get_logger().info(f'Starting Closed Loop Figure-8: scale={scale}, radius={radius}')

        success = await self.run_motion("move", radius, speed, 0.0, 0.0, 0.0, p_control_mode, goal_handle)
        if not success:
            if goal_handle.is_cancel_requested:
                goal_handle.canceled()
            else:
        ...
        for i in range(3):
            success = await self.run_motion("move", scale, speed, 0.0, 0.0, 0.0, p_control_mode, goal_handle)
            if not success:
                if goal_handle.is_cancel_requested:
                    goal_handle.canceled()
                else:
        ...
        success = await self.run_motion("move", conn_dist, speed, 0.0, 0.0, 0.0, p_control_mode, goal_handle)
        if not success:
            if goal_handle.is_cancel_requested:
                goal_handle.canceled()
            else:
        ...
        success = await self.run_motion("move", final_dist, speed, 0.0, 0.0, 0.0, p_control_mode, goal_handle)
        if not success:
            if goal_handle.is_cancel_requested:
                goal_handle.canceled()
            else:
...
def main(args=None):
    ...
    rclpy.spin(ClosedLoopShapeNode(), executor=MultiThreadedExecutor())
    rclpy.shutdown()
